...
rm -f "${PACKAGENAME}"
fpm -s dir -t rpm  -p ${PACKAGENAME} -n ${NAME} -v ${VERSION} --iteration ${RELEASE} -a ${ARCH} -m "${MAINTAINER}" --rpm-summary "${SUMMARY}" --description "${SUMMARY}" --rpm-defattrfile "$DEFATTRFILE" --url "${URL}" --vendor "$VENDOR" --license "$LICENSE" -C $TMPDIR s5cmd=${BINDIR}/${NAME}
