...
ARCH="noarch"
DISTTAG="el8"
PACKAGENAME="${NAME}-${VERSION}-${RELEASE}.$ARCH.$DISTTAG.rpm"
...
/usr/bin/ln -s $AWSDIR/v$MAJORVERSION/${VERSION} $AWSDIR/v$MAJORVERSION/current 
/usr/bin/ln -s $AWSDIR/v$MAJORVERSION/current/bin/aws $BINDIR/aws
/usr/bin/ln -s $AWSDIR/v$MAJORVERSION/current/bin/aws_completer $BINDIR/aws_completer
EOF
cat <<EOF>"$TMPFILE_AFTER_REMOVE"
/usr/bin/rm -f $BINDIR/aws
/usr/bin/rm -f $BINDIR/aws_completer
/usr/bin/rm -f $AWSDIR/v$MAJORVERSION/current
EOF
################
# install to 
# BUILDIDR
...
/usr/bin/rm -f "${PACKAGENAME}"
#
# Add "-e" to fpm for editing SPEC in place
#
fpm -s dir -t rpm  -p ${PACKAGENAME} -n ${NAME} -v ${VERSION} --iteration ${RELEASE} -a ${ARCH} -m "${MAINTAINER}" --url ${URL} --rpm-summary "${SUMMARY}" --description "${SUMMARY}" --after-install "$TMPFILE_AFTER_INSTALL" --after-remove "$TMPFILE_AFTER_REMOVE" --vendor "$VENDOR" --license "$LICENSE" --rpm-changelog "$TMPFILE_CHANGELOG" -C "$TMPDIR_BUILD"
